...
s.linkInternalFilters="javascript:,conserveonline.org"
...
function s_doPlugins(s) {
	...
	s.prop14=s.getTimeParting('h','-5','2007'); // Set hour 
	s.eVar7 = s.getValOnce(s.prop14,'s_eVar7',0);
	
	s.prop15=s.getTimeParting('d','-5','2007'); // Set day
	s.eVar8 = s.getValOnce(s.prop15,'s_eVar8',0);
	
	s.prop16=s.getTimeParting('w','-5','2007'); // Set Weekend / Weekday
	s.eVar9=s.getValOnce(s.prop16,'s_eVar9',0);
	
	/* Set New Repeat Variable*/
	s.prop13 = s.getNewRepeat();
	s.eVar6 = s.getValOnce(s.prop13,'s_eVar6',0);
	...
	/*User Type Pathing*/
	if(s.prop6&&!s.prop7)s.prop7=s.prop6+":"+s.pageName;
	
	/*Set Visit Number Variable*/
	s.prop17 = s.getVisitNum();
	
	/*Pathing - Page Views by New/Repeat User*/
	s.prop18 = s.setCustomPath(s.eVar6,'s_eVar6','Status=',s.pageName);
...
//s.trackingServer="metrics.conserveonline.org"
//s.trackingServerSecure="smetrics.conserveonline.org"
s.dc=112
