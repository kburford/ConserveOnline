tinyMCE.importPluginLanguagePack('tnc_advimage');
     ...
     getInfo:function(){
   	...
   	version : tinyMCE.majorVersion + '.' + tinyMCE.minorVersion}},
                    getControlHTML:  function(cn){
                        switch(cn){
                            case"tnc_advimage": return tinyMCE.getButtonHTML(cn,'lang_image_desc','{$themeurl}/images/image.gif','mceAdvImage')
                        }
                        return ""
                     },
...
//!!!!! 
 			template['file']   = '../../plugins/tnc_advimage/image.htm';
 			template['width']  = 480;
 			template['height'] = 575;
                     ...
                     cleanup: function(type,content){
                          switch(type){
                               case"insert_to_editor_dom":var imgs=content.getElementsByTagName("img"),src,i;for(i=0;i<imgs.length;i++){
                      ...
                      handleNodeChange:function(editor_id,node,undo_index,undo_levels,visual_aid,any_selection){
                         ...
                         while((node=node.parentNode));tinyMCE.switchClass(editor_id+'_advimage','mceButtonNormal');
                         return true
                      },
                      _getImageSrc:function(s){
                           var sr,p=-1;if(!s)return"";
                           if((p=s.indexOf('this.src='))!=-1){sr=s.substring(p+10);
                            sr=sr.substring(0,sr.indexOf('\''));
                            return sr}return""
                     }
           };
tinyMCE.addPlugin("tnc_advimage",TinyMCE_ThriveSmartAdvancedImagePlugin);